 ...
 */
    ...
    class public_ods_tipo_personaPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('public_ods_tipo_personassearch', $this->dataset,
                array('id_tipo_persona', 'no_tipo_persona'),
                array($this->RenderText('Id Tipo Persona'), $this->RenderText('Nombre Tipo Persona')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('public_ods_tipo_personaasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('id_tipo_persona', $this->RenderText('Id Tipo Persona')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('no_tipo_persona', $this->RenderText('Nombre Tipo Persona')));
        }
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('public_ods_tipo_personaGrid_no_tipo_persona_handler_view');
            $grid->AddSingleRecordViewColumn($column);
        }
        ...
        {
            //
            // Edit column for no_tipo_persona field
            //
            $editor = new TextEdit('no_tipo_persona_edit');
            $editor->SetSize(100);
            $editor->SetMaxLength(100);
            $editColumn = new CustomEditColumn('Nombre Tipo Persona', 'no_tipo_persona', $editor, $this->dataset);
        ...
        public function GetModalGridDeleteHandler() { return 'public_ods_tipo_persona_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'public_ods_tipo_personaGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('no_tipo_persona', 'Nombre Tipo Persona', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'public_ods_tipo_personaGrid_no_tipo_persona_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for no_tipo_persona field
            //
            $column = new TextViewColumn('no_tipo_persona', 'Nombre Tipo Persona', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'public_ods_tipo_personaGrid_no_tipo_persona_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
