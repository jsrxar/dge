 ...
 */
    ...
    class public_ods_bienPage extends Page
    {
        protected function DoBeforeCreate()
        {
            ...
            $this->dataset->AddLookupField('id_espacio', 'public.ods_espacio', new IntegerField('id_espacio', null, null, true), new StringField('co_espacio', 'id_espacio_co_espacio', 'id_espacio_co_espacio_public_ods_espacio'), 'id_espacio_co_espacio_public_ods_espacio');
        }
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('public_ods_bienssearch', $this->dataset,
                array('id_bien', 'id_tipo_bien_ds_referencia', 'id_espacio_co_espacio', 'ds_observacion'),
                array($this->RenderText('Id Bien'), $this->RenderText('Id Tipo Bien'), $this->RenderText('Id Espacio'), $this->RenderText('Ds Observacion')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('public_ods_bienasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateLookupSearchInput('id_espacio', $this->RenderText('Id Espacio'), $lookupDataset, 'id_espacio', 'co_espacio', false, 8));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('ds_observacion', $this->RenderText('Ds Observacion')));
        }
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            $column->SetFullTextWindowHandlerName('public_ods_bienGrid_ds_observacion_handler_list');
            $column->SetDescription($this->RenderText('Observaciones del bien.'));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        }
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            $editColumn = new CustomEditColumn('Ds Observacion', 'ds_observacion', $editor, $this->dataset);
        ...
        public function GetModalGridDeleteHandler() { return 'public_ods_bien_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'public_ods_bienGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('ds_observacion', 'Ds Observacion', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'public_ods_bienGrid_ds_observacion_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for ds_observacion field
            //
            $column = new TextViewColumn('ds_observacion', 'Ds Observacion', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'public_ods_bienGrid_ds_observacion_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
    ...
    try
    {
        $Page = new public_ods_bienPage("bien.php", "public_ods_bien", GetCurrentUserGrantForDataSource("public.ods_bien"), 'UTF-8');
        $Page->SetShortCaption('Bien');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Bien');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("public.ods_bien"));
