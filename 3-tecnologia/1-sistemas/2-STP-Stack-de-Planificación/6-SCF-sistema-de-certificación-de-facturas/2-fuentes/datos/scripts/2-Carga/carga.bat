...
for %%f in (C:\archivos\facturas\inicial\*.csv) do (
	@echo Cargando "%%~nxf"
	C:\app\pentaho-ee\postgresql\bin\psql -h %PGHOST% -d %PDBASE% -U %PGUSER% -c "\COPY stg_base_rrhh(ds_ministerio,ds_secretaria,ds_subsecretaria,ds_direccion_area,ds_area_dependencia,ds_sector,ds_sub_sector,ds_puesto,ds_funcion,ds_ubicacion_fisica,ds_apellido_nombres,ds_dni,ds_nro,ds_cuit,ds_f_nacimiento,ds_lm_at,ds_tipo,ds_ingreso,ds_vto_contrato,ds_estudios,ds_fin_contrato,ds_categ_lm,ds_universidad,ds_convenio_at,ds_julio_10p,ds_agosto_14p,ds_sept,ds_oct,ds_nov,ds_dic) FROM '%%~dpnxf' DELIMITERS ';'"
)
