 ...
 */
    ...
    class public_ods_areaPage extends Page
    {
        protected function DoBeforeCreate()
        {
            ...
            $this->dataset->AddField($field, false);
            $this->dataset->AddLookupField('id_area_padre', 'public.ods_area', new IntegerField('id_area', null, null, true), new StringField('ds_referencia', 'id_area_padre_ds_referencia', 'id_area_padre_ds_referencia_public_ods_area'), 'id_area_padre_ds_referencia_public_ods_area');
        }
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('public_ods_areassearch', $this->dataset,
                array('id_area', 'no_area', 'id_area_padre_ds_referencia', 'ds_referencia'),
                array($this->RenderText('Id Area'), $this->RenderText('Nombre Area'), $this->RenderText('Id Area Padre'), $this->RenderText('Ds Referencia')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('public_ods_areaasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('id_area', $this->RenderText('Id Area')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('no_area', $this->RenderText('Nombre Area')));
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateLookupSearchInput('id_area_padre', $this->RenderText('Id Area Padre'), $lookupDataset, 'id_area', 'ds_referencia', false, 8));
        ...
        {
            //
            // View column for id_area field
            //
            $column = new TextViewColumn('id_area', 'Id Area', $this->dataset);
            ...
            //
            $column = new TextViewColumn('no_area', 'Nombre Area', $this->dataset);
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('public_ods_areaGrid_no_area_handler_list');
            $column->SetDescription($this->RenderText('Nombre de Área.'));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
            ...
            //
            $column = new TextViewColumn('id_area_padre_ds_referencia', 'Id Area Padre', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText('Identificador único de Área Padre.'));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('public_ods_areaGrid_ds_referencia_handler_list');
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('public_ods_areaGrid_no_area_handler_view');
            $grid->AddSingleRecordViewColumn($column);
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('public_ods_areaGrid_ds_referencia_handler_view');
            $grid->AddSingleRecordViewColumn($column);
        }
        ...
        {
            //
            // Edit column for no_area field
            //
            $editor = new TextEdit('no_area_edit');
            $editor->SetSize(100);
            $editor->SetMaxLength(100);
            $editColumn = new CustomEditColumn('Nombre Area', 'no_area', $editor, $this->dataset);
            ...
            //
            // Edit column for id_area_padre field
            //
            $editor = new ComboBox('id_area_padre_edit', $this->GetLocalizerCaptions()->GetMessageString('PleaseSelect'));
            $lookupDataset = new TableDataset(
                new PgConnectionFactory(),
                GetConnectionOptions(),
                '"public"."ods_area"');
            ...
            $editColumn = new LookUpEditColumn(
                ...
                'id_area_padre', 
                $editor, 
                $this->dataset, 'id_area', 'ds_referencia', $lookupDataset);
        ...
        protected function AddPrintColumns(Grid $grid)
        {
            ...
            // View column for no_area field
            //
            $column = new TextViewColumn('no_area', 'No Area', $this->dataset);
            $column->SetOrderable(true);
            $grid->AddPrintColumn($column);
        ...
        public function GetModalGridDeleteHandler() { return 'public_ods_area_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'public_ods_areaGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('no_area', 'Nombre Area', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'public_ods_areaGrid_no_area_handler_list', $column);
            ...
            $column = new TextViewColumn('ds_referencia', 'Ds Referencia', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'public_ods_areaGrid_ds_referencia_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for no_area field
            //
            $column = new TextViewColumn('no_area', 'Nombre Area', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'public_ods_areaGrid_no_area_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            //
            // View column for ds_referencia field
            //
            $column = new TextViewColumn('ds_referencia', 'Ds Referencia', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'public_ods_areaGrid_ds_referencia_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
    ...
    try
    {
        $Page = new public_ods_areaPage("area.php", "public_ods_area", GetCurrentUserGrantForDataSource("public.ods_area"), 'UTF-8');
        $Page->SetShortCaption('Area');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Area');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("public.ods_area"));
