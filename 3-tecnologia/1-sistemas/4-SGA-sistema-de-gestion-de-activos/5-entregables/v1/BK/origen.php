 ...
 */
    ...
    class public_ods_origenPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('public_ods_origenssearch', $this->dataset,
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('public_ods_origenasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            $column->SetFullTextWindowHandlerName('public_ods_origenGrid_no_origen_handler_list');
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('public_ods_origenGrid_no_origen_handler_view');
            $grid->AddSingleRecordViewColumn($column);
        }
        ...
        public function GetModalGridDeleteHandler() { return 'public_ods_origen_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'public_ods_origenGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('no_origen', 'Nombre Origen', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'public_ods_origenGrid_no_origen_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for no_origen field
            //
            $column = new TextViewColumn('no_origen', 'Nombre Origen', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'public_ods_origenGrid_no_origen_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
    ...
    try
    {
        $Page = new public_ods_origenPage("origen.php", "public_ods_origen", GetCurrentUserGrantForDataSource("public.ods_origen"), 'UTF-8');
        $Page->SetShortCaption('Origen');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Origen');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("public.ods_origen"));
