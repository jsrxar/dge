 ...
 */
    ...
    class public_ods_origen_lecturaPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('public_ods_origen_lecturassearch', $this->dataset,
                array('id_origen_lectura', 'co_origen_lectura', 'no_origen_lectura'),
                array($this->RenderText('Id Origen Lectura'), $this->RenderText('Código Origen Lectura'), $this->RenderText('Nombre Origen Lectura')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('public_ods_origen_lecturaasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('id_origen_lectura', $this->RenderText('Id Origen Lectura')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('co_origen_lectura', $this->RenderText('Código Origen Lectura')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('no_origen_lectura', $this->RenderText('Nombre Origen Lectura')));
        }
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            $editor = new TextEdit('id_origen_lectura_edit');
            $editColumn = new CustomEditColumn('Id Origen Lectura', 'id_origen_lectura', $editor, $this->dataset);
            ...
            //
            // Edit column for co_origen_lectura field
            //
            $editor = new TextEdit('co_origen_lectura_edit');
            $editor->SetSize(1);
            $editor->SetMaxLength(1);
            $editColumn = new CustomEditColumn('Código Origen Lectura', 'co_origen_lectura', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
            ...
            //
            // Edit column for no_origen_lectura field
            //
            $editor = new TextEdit('no_origen_lectura_edit');
            $editor->SetSize(100);
            $editor->SetMaxLength(100);
            $editColumn = new CustomEditColumn('Nombre Origen Lectura', 'no_origen_lectura', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
        ...
        public function GetModalGridDeleteHandler() { return 'public_ods_origen_lectura_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'public_ods_origen_lecturaGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('no_origen_lectura', 'Nombre Origen Lectura', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'public_ods_origen_lecturaGrid_no_origen_lectura_handler_list', $column);
