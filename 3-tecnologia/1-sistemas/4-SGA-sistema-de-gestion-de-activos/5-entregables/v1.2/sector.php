 ...
 */
    ...
    class public_sga_sectorPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('public_sga_sectorssearch', $this->dataset,
                array('id_sector', 'no_sector'),
                array($this->RenderText('Id Sector'), $this->RenderText('No Sector')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('public_sga_sectorasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('id_sector', $this->RenderText('Id Sector')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('no_sector', $this->RenderText('No Sector')));
        }
        ...
        {
            //
            // View column for id_sector field
            //
            $column = new TextViewColumn('id_sector', 'Id Sector', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText('Identificador único del sector.'));
            ...
            //
            $column = new TextViewColumn('no_sector', 'No Sector', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText('Nombre del sector (noble, ibndustrial, etc.).'));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        }
        ...
        {
            //
            // Edit column for no_sector field
            //
            $editor = new TextEdit('no_sector_edit');
            $editor->SetSize(50);
            $editor->SetMaxLength(50);
            $editColumn = new CustomEditColumn('No Sector', 'no_sector', $editor, $this->dataset);
        ...
        public function GetModalGridViewHandler() { return 'public_sga_sector_inline_record_view'; }
        ...
        public function GetModalGridDeleteHandler() { return 'public_sga_sector_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'public_sga_sectorGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new public_sga_sectorPage("sector.php", "public_sga_sector", GetCurrentUserGrantForDataSource("public.sga_sector"), 'UTF-8');
        $Page->SetShortCaption('Sector');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Sector');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("public.sga_sector"));
