...
module Redmine
  module FieldFormat
    ...
    class Base
      ...
      # %value_id% => the custom field id
      # %value_id_ori% => the source id of the custom field
      # %value_position% => the custom field position
      ...
      def url_from_pattern(custom_field, value, customized)
        ...
        url.gsub!('%value_id%') {value.id.to_s}
        url.gsub!('%value_id_ori%') {value.id_ori.to_s}
        url.gsub!('%value_position%') {value.position.to_s}
