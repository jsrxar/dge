 ...
 */
    ...
    class public_ods_lugarPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('public_ods_lugarssearch', $this->dataset,
                array('id_lugar', 'co_lugar', 'no_lugar', 'ds_direccion'),
                array($this->RenderText('Id Lugar'), $this->RenderText('Código Lugar'), $this->RenderText('Nombre Lugar'), $this->RenderText('Direccion')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('public_ods_lugarasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('id_lugar', $this->RenderText('Id Lugar')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('co_lugar', $this->RenderText('Código Lugar')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('no_lugar', $this->RenderText('Nombre Lugar')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('ds_direccion', $this->RenderText('Direccion')));
        }
        ...
        {
            //
            // Edit column for co_lugar field
            //
            $editor = new TextEdit('co_lugar_edit');
            $editor->SetSize(1);
            $editor->SetMaxLength(1);
            $editColumn = new CustomEditColumn('Código Lugar', 'co_lugar', $editor, $this->dataset);
            ...
            //
            // Edit column for no_lugar field
            //
            $editor = new TextEdit('no_lugar_edit');
            $editor->SetSize(100);
            $editor->SetMaxLength(100);
            $editColumn = new CustomEditColumn('Nombre Lugar', 'no_lugar', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
            ...
            //
            $editor = new TextEdit('ds_direccion_edit');
            $editor->SetSize(100);
            $editor->SetMaxLength(100);
            $editColumn = new CustomEditColumn('Direccion', 'ds_direccion', $editor, $this->dataset);
        ...
        public function GetModalGridDeleteHandler() { return 'public_ods_lugar_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'public_ods_lugarGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('no_lugar', 'Nombre Lugar', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'public_ods_lugarGrid_no_lugar_handler_list', $column);
            ...
            $column = new TextViewColumn('ds_direccion', 'Direccion', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'public_ods_lugarGrid_ds_direccion_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for no_lugar field
            //
            $column = new TextViewColumn('no_lugar', 'Nombre Lugar', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'public_ods_lugarGrid_no_lugar_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            //
            // View column for ds_direccion field
            //
            $column = new TextViewColumn('ds_direccion', 'Direccion', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'public_ods_lugarGrid_ds_direccion_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
    ...
    try
    {
        $Page = new public_ods_lugarPage("lugar.php", "public_ods_lugar", GetCurrentUserGrantForDataSource("public.ods_lugar"), 'UTF-8');
        $Page->SetShortCaption('Lugar');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Lugar');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("public.ods_lugar"));
