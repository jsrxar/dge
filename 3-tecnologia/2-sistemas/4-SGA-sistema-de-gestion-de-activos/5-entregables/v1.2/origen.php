 ...
 */
    ...
    class public_sga_origenPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('public_sga_origenssearch', $this->dataset,
                array('id_origen', 'no_origen'),
                array($this->RenderText('Id Origen'), $this->RenderText('Nombre Origen')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('public_sga_origenasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('id_origen', $this->RenderText('Id Origen')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('no_origen', $this->RenderText('Nombre Origen')));
        }
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('public_sga_origenGrid_no_origen_handler_view');
            $grid->AddSingleRecordViewColumn($column);
        }
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            $editor = new TextEdit('id_origen_edit');
            $editColumn = new CustomEditColumn('Id Origen', 'id_origen', $editor, $this->dataset);
            ...
            //
            // Edit column for no_origen field
            //
            $editor = new TextEdit('no_origen_edit');
            $editor->SetSize(100);
            $editor->SetMaxLength(100);
            $editColumn = new CustomEditColumn('Nombre Origen', 'no_origen', $editor, $this->dataset);
        ...
        protected function AddPrintColumns(Grid $grid)
        {
            ...
            // View column for no_origen field
            //
            $column = new TextViewColumn('no_origen', 'No Origen', $this->dataset);
            $column->SetOrderable(true);
            $grid->AddPrintColumn($column);
        }
        ...
        public function GetModalGridViewHandler() { return 'public_sga_origen_inline_record_view'; }
        ...
        public function GetModalGridDeleteHandler() { return 'public_sga_origen_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'public_sga_origenGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('no_origen', 'Nombre Origen', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'public_sga_origenGrid_no_origen_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for no_origen field
            //
            $column = new TextViewColumn('no_origen', 'Nombre Origen', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'public_sga_origenGrid_no_origen_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
    ...
    try
    {
        $Page = new public_sga_origenPage("origen.php", "public_sga_origen", GetCurrentUserGrantForDataSource("public.sga_origen"), 'UTF-8');
        $Page->SetShortCaption('Origen');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Origen');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("public.sga_origen"));
